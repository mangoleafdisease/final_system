...
<head>
    ...
    <title>Expandable Product Card</title>
...
<?php foreach ($groupedProducts as $index => $product): ?>
    <div class="card" onclick="toggleSizes('<?php echo htmlspecialchars($index); ?>')">
        ...
        <div class="sizes" id="sizes-<?php echo htmlspecialchars($index); ?>" style="display:none;">
            <?php 
                // Use the price from the current product
                $item_price = floatval($product['price']); // Assuming the price is stored in the product array
...
<script>
    ...
    function toggleSizes(index) {
        ...
        sizesDiv.style.display = sizesDiv.style.display === 'none' ? 'block' : 'none';
    }
