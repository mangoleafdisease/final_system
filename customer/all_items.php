...
$items = [];
try {
    $query = "SELECT id, name, size, quantity, old_price, price, image, year_level, category FROM inventory";
        ...
        <div class="search-bar">
            <form method="GET" action="">
                ...
                <select name="category" class="select">
                    <option value="" disabled selected>-- List of Items --</option>
                    ...
                    <?php foreach ($items as $item): ?>
                        <div class="item-card">
                            ...
                            <div class="item-details">
                                ...
                                <div class="item-name">Quantity :<?php echo htmlspecialchars($item['quantity']); ?></div>
                                <div class="item-size">Size:  <?php echo htmlspecialchars($item['size']); ?></div>
                                <?php if (!empty($item['old_price'])): ?>
                                <div class="item-price" style="text-decoration: line-through; color: #f12312;">
                                    Old Price: ₱<?php echo htmlspecialchars($item['old_price']); ?>
                                </div>
                            <?php endif; ?>


                                <div class="item-price">Price: ₱<?php echo htmlspecialchars($item['price']); ?></div>
                            </div>
...
<script>
    ...
    if (cart.length === 0) {
        Swal.fire({
            ...
            text: 'Please add items before saving your cart.',
        });
        return;
    }
    ...
    .then(data => {
        if (data.status === 'success') {
            Swal.fire({
                icon: 'success',
                title: 'Cart Saved!',
                text: 'Your cart has been successfully saved.',
            });
            cart.length = 0;  
            displayCart();  
        } else {
